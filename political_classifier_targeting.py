...
clf = Pipeline(
    [('vect', CountVectorizer(tokenizer=Tokenizer(), ngram_range=(1, 2), max_features=5000, stop_words='english')),
     ('tfidf', TfidfTransformer()),
     ('clf', LogisticRegression(class_weight='balanced'))])
...
print("AUC [train]: ", str(roc_auc_score(train['target'], y_predict_train)))
print("AUC [test]: ", str(roc_auc_score(test['target'], y_predict_test)))

print("F1 score [train]: ", str(f1_score(train['target'], y_predict_train)))
print("F1 score [test]: ", str(f1_score(test['target'], y_predict_test)))
